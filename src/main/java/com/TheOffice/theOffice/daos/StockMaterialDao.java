...
public class StockMaterialDao {
    ...
    public StockMaterial findById(Long id) {
        ...
        return jdbcTemplate.query(sql,stockMaterialRowMapper)
                .stream()
                .findFirst()
                .orElseThrow(()-> new RuntimeException("Stock des produits non trouvé"));
    }
    ...
    public StockMaterial update(Long id, StockMaterial stockMaterial) {
        if (!stockMaterialExists(id)) {
            throw new RuntimeException("Stock des produits avec l'ID : " + id + " n'existe pas");
        }
        ...
        if (rowsAffected <= 0) {
            throw new RuntimeException("Échec de la mise à jour du stock des produits avec l'ID : " + id);
        }
