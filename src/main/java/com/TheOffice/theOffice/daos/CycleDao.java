...
public class CycleDao {
    ...
    public Cycle findById(Long id) {
        ...
        return jdbcTemplate.query(sql, cycleRowMapper, id)
                .stream()
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Cycle non trouvée"));
    }
    ...
    public Cycle update(Long id, Cycle cycle) {
        if (!cycleExists(id)) {
            throw new RuntimeException("Cycle avec l'ID : " + id + " n'existe pas");
        }
        ...
        if (rowsAffected <= 0) {
            throw new RuntimeException("Échec de la mise à jour du cycle avec l'ID : " + id);
        }
        return cycle;
    }
